package app.ehtudo.iptv.ui.favorites
...
/**
 * Standalone favorites body — segmented switcher (Live / Movie / Series)
 * plus a grid for the active segment. Each grid pulls a Flow from
 * [app.ehtudo.iptv.data.FavoriteRepository]; Room auto-refreshes when the
 * user toggles a favorite from any detail screen.
 *
 * Renders without its own [androidx.compose.material3.Scaffold] so it can
 * sit inside the dashboard's [HorizontalPager] page; the dashboard owns
 * the top/bottom bars. iOS counterpart: `FavoritesView`.
 */
...
fun FavoritesBody(
    ...
    var selected by remember { mutableStateOf(FavoriteType.VOD) }
